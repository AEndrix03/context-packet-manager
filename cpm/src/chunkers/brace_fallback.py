...
from chunkers.token_budget import TokenBudgeter, Block


def _split_by_braces(text: str) -> List[str]:
...
class BraceFallbackChunker:
    ...
    def chunk(
        ...
        **kwargs: Any,
        ...
        parts = _split_by_braces(text)
        blocks: List[Block] = []

        for i, p in enumerate(parts):
            parent_id = f"{source_id}:brace:part:{i}"
            parent_meta: Dict[str, Any] = {"kind": "brace_part", "level": config.parent_level_name, "parent_id": parent_id}
