...
class ModelRuntime:
    def __init__(self, spec: ModelSpec, global_sem: asyncio.Semaphore):
        ...
        self.queue: asyncio.Queue[WorkItem] = asyncio.Queue(maxsize=int(spec.queue.max_size))
    ...
    async def enqueue(self, texts: List[str], options: Dict[str, Any]) -> Tuple[np.ndarray, int, Dict[str, Any]]:
        ...
        fut: asyncio.Future[Tuple[np.ndarray, int, Dict[str, Any]]] = asyncio.get_event_loop().create_future()
        item = WorkItem(
            model=self.spec.model,
            texts=texts,
            options=options,
    ...
    async def _add_replica_locked(self) -> None:
        ...
        asyncio.create_task(self._replica_loop(rep))
