...
""",
    ...
    print(f"[src] initialized at {root.as_posix()}")
...
def cmd_register(args) -> None:
    ...
    if args.type == "http":
        if not args.base_url:
            raise SystemExit("[src] register http requires --base-url")
        ...
        raise SystemExit(f"[src] unsupported type {args.type!r}")

    if _probe(url):
        r = requests.post(f"{url.rstrip('/')}/models/register", json=payload, timeout=5.0)
        if not r.ok:
            raise SystemExit(f"[src] register failed: {r.status_code} {r.text}")
        print(f"[src] registered {args.model}")
        return
    ...
    print(f"[src] registered (offline) {args.model}")
...
def cmd_alias(args) -> None:
    ...
    if _probe(url):
        ...
        if not r.ok:
            raise SystemExit(f"[src] alias failed: {r.status_code} {r.text}")
        print("[src] ok")
        return
    ...
    for m in pf.models:
        if m.model == args.model or (m.alias and m.alias == args.model):
            ...
            ok = True
    if not ok:
        raise SystemExit(f"[src] unknown model/alias: {args.model}")
    save_pool(cfg.pool_yml, pf)
    print("[src] ok")
...
def cmd_enable(args, enabled: bool) -> None:
    ...
    if _probe(url):
        r = requests.post(f"{url.rstrip('/')}/models/enable", json={"model": args.model, "enabled": enabled},
                          timeout=5.0)
        if not r.ok:
            raise SystemExit(f"[src] enable/disable failed: {r.status_code} {r.text}")
        print("[src] ok")
        return
...
def cmd_unregister(args) -> None:
    ...
    if _probe(url):
        ...
        if not r.ok:
            raise SystemExit(f"[src] unregister failed: {r.status_code} {r.text}")
        print("[src] ok")
        return
...
def cmd_pool_start(args) -> None:
    ...
    if _probe(url):
        print(f"[src] already running at {url}")
        return
    ...
    if args.detach:
        ...
        p = subprocess.Popen(
            [sys.executable, "-m", "server", "--config", cfg_path],
        ...
        print(f"[src] started pid={p.pid}")
        return

    subprocess.run([sys.executable, "-m", "server", "--config", cfg_path], check=False)
...
def cmd_pool_stop(args) -> None:
    ...
    if not pidfile.exists():
        print("[src] no pidfile")
        return
    try:
        pid = int(pidfile.read_text(encoding="utf-8").strip())
    except Exception:
        print("[src] invalid pidfile")
        return
...
def cmd_pool_health(args) -> None:
    cfg_path = _cfg_path(args)
    url = _pool_url(cfg_path)
    print(f"[src] {'OK' if _probe(url) else 'DOWN'} {url}")
